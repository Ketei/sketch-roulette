...
func _init() -> void:
	...
	_browse_button.custom_minimum_size = Vector2(32.0, 32.0)
...
func _ready() -> void:
	...
	new_container.add_child(_browse_button)
	
	_ok_button = get_ok_button()
	
	_browse_button.tooltip_text = "Browse Files"
	_browse_button.icon = preload("res://icons/folder_icon.svg")
...
func _on_browse_pressed() -> void:
	var browser := preload("res://scripts/image_browser.gd").new()
	add_child(browser)
	browser.popup()
	
	var result: Array = await browser.dialog_finished
	
	if result[0]:
		_dialog_line.text = result[1]
		_on_text_changed(_dialog_line.text)
	browser.queue_free()
